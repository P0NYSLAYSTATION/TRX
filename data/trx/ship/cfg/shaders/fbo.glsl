...
vec4 ps1QuantizePremultiplied(vec4 color)
{
    if (color.a <= 0.0) {
        return color;
    }
    vec3 channel = clamp(color.rgb / color.a, 0.0, 1.0) * 255.0;
    color.rgb = floor(channel / 8.0) / 31.0 * color.a;
    return color;
}
...
void main(void) {
    ...
    if (uDitherMode == DITHER_MODE_SOFTWARE_RENDERER) {
        ...
        outColor = ps1QuantizePremultiplied(outColor);
    } else if (uDitherMode == DITHER_MODE_PS1_POST_PROCESS) {
        outColor.rgb = dither(
            outColor.rgb, vertTexCoords * vec2(textureSize(uTex0, 0)),
            DITHER_STEPS_15_BIT);
    }
